...
import { getSchoolInboxThreads, type SchoolInboxThread } from '@/features/tickets/messages-unread-school'
...
export default async function SchoolMessagesPage() {
  ...
  const threads = await getSchoolInboxThreads(supabase)
      ...
      {threads.length === 0 ? (
        <div className="card border-dashed px-4 py-10 text-center">
          ...
          <p className="mt-1 text-xs text-slate-500">
            Les échanges avec vos clients et l&apos;atelier apparaîtront ici.
          </p>
        </div>
      ) : (
...
function SchoolThreadRow({ thread }: { thread: SchoolInboxThread }) {
